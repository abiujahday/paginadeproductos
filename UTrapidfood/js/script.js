$( document ).ready(function() {
	...
	function checkPassword() {
		...
		if(password ==""){
			...
			if (password.length < 6) {
				document.getElementById("passmsg").innerHTML = "La contraseña debe de tener por lo menos 6 caracteres";
				$("#passmsg").show();
			}else {
	...
	$("#sendRegister").submit(function(e) {
		...
		$.post("registerDB.php",DATOS,function(respuesta) {
			if (respuesta == "OK") {
				$("#passrepmsg").html("Tu cuenta se ha registrado correctamente, pero para poder acceder a la plataforma tendrás que esperar a que un administrador asigne el tipo de usuario que desempeñarás.");
